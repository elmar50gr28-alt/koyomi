(function (root, factory) {
  ...
  function applyDivination(text,input){const layer=build(input),technical=/確度|計算|命式|配置|内訳|データ|方式|注意事項/;let output=String(text||'').replace(/【([^】]+)】\n([\s\S]*?)(?=\n\n【|$)/g,(all,heading,body)=>`【${heading}】\n${technical.test(heading)?body:conversationalize(body)}`);const evidence=(input.evidence||[]).filter(Boolean).slice(0,2).join('／'),action=input.action||'';const intro=`【${input.system||'この占術'}を姐さんが読むわ】\n${layer.opening}\n${layer.address}${evidence?`\n今ここで効いている根拠は「${evidence}」。`:''}`;const turn=`${layer.bridge}${action?` だから最初の一手は「${action}」よ。`:''}`;output=`${intro}\n\n${turn}\n\n${output}`;if(layer.aside)output+=`\n\n【姐さんのひと言】\n${layer.aside}`;output+=`\n\n【姐さんの締め】\n${layer.closing}`;return{text:output,persona:layer}}
  return{BANK,build,apply,applyDivination,conversationalize,topic};
});
