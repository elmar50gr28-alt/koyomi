(function(root,factory){root.KOYOMI_PERSONA_RENDERER=factory()})(typeof globalThis!=='undefined'?globalThis:this,function(){
 ...
 function render(model,{voice='sister'}={}){const v=voices.get(voice)||voices.get('sister');if(!v)throw new Error(`unknown voice: ${voice}`);const lex=globalThis.KOYOMI_SISTER_LEXICON?.session([model.system,model.scenario?.state,(model.evidence||[]).join('|'),voice].join('|')),phrase=(group,offset)=>lex?.pick(group,offset)||'',context={...model,voice:v,phrase};return(model.order||['opening','result','scenario','action','stop','review','evidence','closing']).map(name=>sections.get(name)?.(context)).filter(Boolean).join('\n\n')}
