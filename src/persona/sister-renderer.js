(function(root,factory){root.KOYOMI_PERSONA_RENDERER=factory()})(typeof globalThis!=='undefined'?globalThis:this,function(){
 ...
 function render(model,{voice='sister',level='standard'}={}){const selected=level==='beginner'?'beginner':voice,v=voices.get(selected)||voices.get('sister');if(!v)throw new Error(`unknown voice: ${selected}`);const lex=globalThis.KOYOMI_SISTER_LEXICON?.session([model.system,model.scenario?.state,(model.evidence||[]).join('|'),selected].join('|')),phrase=(group,offset)=>lex?.pick(group,offset)||'',context={...model,voice:v,phrase,level,plain};const order=model.order||(level==='beginner'?['opening','result','action','stop','review','evidence','closing']:['opening','result','scenario','action','stop','review','evidence','closing']);return order.map(name=>sections.get(name)?.(context)).filter(Boolean).join('\n\n')}
 ...
 registerSection('evidence',c=>`【${c.voice.evidence}】\n${c.level==='beginner'?'占いの結果を普通の言葉に直すと、こういうことよ。':c.phrase('evidence')}\n${c.level==='beginner'?c.plain((c.evidence||[]).filter(Boolean).join('／')||'複数の計算結果を確認済み'):((c.evidence||[]).filter(Boolean).join('／')||'算出結果を確認済み')}。`);
