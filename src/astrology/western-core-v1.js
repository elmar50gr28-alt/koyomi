...
function build(input={}){const raw=input.placements||input.planets||{},placements={};Object.entries(raw).forEach(([name,value])=>{const lon=typeof value==='number'?value:value?.lon;if(Number.isFinite(lon))placements[name]={...(typeof value==='object'?value:{}),lon:mod(lon),sign:signOf(lon),degree:degreeOf(lon)}});return{input:input.input||{},calculationMeta:{engine:input.engine||'koyomi-offline',offlineCapable:true,birthTimeKnown:input.birthTimeKnown!==false,generatedAt:new Date().toISOString()},natal:{placements,angles:input.angles||{},cusps:input.cusps||[],aspects:input.aspects||aspects(placements)}}}
