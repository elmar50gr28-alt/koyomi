(function(root,factory){
 ...
 function compose(input={}){const normalized={...input,domain:domain(input.domain),state:state(input.state,input.score),seed:[input.seed,input.date,input.domain,input.state,input.variant].join('|')};let best=null;for(let attempt=0;attempt<7;attempt++){const structure=chooseStructure(normalized,attempt),text=render(structure,parts(normalized,attempt)),quality=audit(text,normalized),candidate={text,meta:{structure,attempt,domain:normalized.domain,state:normalized.state,quality}};if(!best||quality.score>best.meta.quality.score)best=candidate;if(quality.pass)return candidate}return best}
