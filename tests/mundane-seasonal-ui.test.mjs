...
assert.ok(app.includes("typeof module.summarizeMonthlyTrend==='function'?module.summarizeMonthlyTrend(trend):mundaneTrendSummary(trend)"), 'an older local helper must not block the annual reading');
assert.ok(app.includes('画面を再読み込みして、もう一度お試しください'), 'local failures must provide an actionable recovery message');
