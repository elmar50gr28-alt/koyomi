...
const [app,worker]=await Promise.all([readFile('app.html','utf8'),readFile('service-worker.js','utf8')]);assert.ok(app.includes('src/reading/integrated-judgment-core.js'));assert.ok(worker.includes('./src/reading/integrated-judgment-core.js'));assert.ok(app.includes('function v202IntegratedJudgment'));assert.ok(app.includes("oracle:['tarot','runes']"));assert.ok(app.includes('r.integratedEvidence=judgment.evidence.map'));assert.ok(app.indexOf('v202IntegratedRenderBase=renderPersonal')<app.indexOf('v200FuriganaRenderBase=renderPersonal'),'integrated scoring must run before final rendering wrappers');
