...
const mapUi=await readFile(new URL('../src/world/world-map-ui.js',import.meta.url),'utf8');assert.ok(mapUi.includes('MAX_VISIBLE_CELLS=500'));assert.ok(mapUi.includes('while(ids.length>=MAX_VISIBLE_CELLS'));assert.ok(mapUi.includes("map.on('moveend',scheduleUpdate)"));assert.ok(mapUi.includes('setTimeout(update,180)'));
