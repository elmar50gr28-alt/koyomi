...
const solar=renderer.build(data,'solar');
assert.equal(solar.implemented,true);
assert.ok(solar.groups.includes('elementEnergy'));
assert.ok(solar.facts.some(x=>x.label==='太陽活動指数'));
assert.ok(solar.source.includes('太陽活動レイヤー'));
...
assert.ok(today.includes('data-layer="solar"'));
assert.ok(today.includes('太陽活動'));
