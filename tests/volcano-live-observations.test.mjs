...
import { FIRMS_SOURCES, isExcluded, normalizeFirmsRows, normalizeUsgsFeatures, summarizeThermal } from '../scripts/update-volcano-observations.mjs';
...
const data=JSON.parse(await readFile(new URL('../data/world/volcano-observations-v1.json',import.meta.url),'utf8'));assert.equal(data.schemaId,'koyomi-volcano-observations-v1');assert.equal(data.freshness.thermalConfigured,false);assert.equal(data.freshness.complete,false);assert.match(data.sha256||'',/^[a-f0-9]{64}$/);
