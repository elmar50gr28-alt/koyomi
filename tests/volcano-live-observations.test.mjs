...
import { FIRMS_SOURCES, assignToVolcanoes, isExcluded, mapConcurrent, normalizeFirmsRows, normalizeUsgsFeatures, summarizeThermal, volcanicTiles } from '../scripts/update-volcano-observations.mjs';
...
const tileVolcanoes=[volcano,{...volcano,id:'west',longitude:91},{...volcano,id:'south',latitude:-7,longitude:110}];assert.equal(volcanicTiles(tileVolcanoes).length,3);const assigned=assignToVolcanoes([{latitude:35.001,longitude:139.001}],tileVolcanoes,15);assert.equal(assigned.fixture.length,1);assert.equal(assigned.west.length,0);
assert.deepEqual(await mapConcurrent([1,2,3,4],2,async value=>value*2),[2,4,6,8]);
...
assert.ok(!script.includes('/world/${FIRMS_DAY_RANGE}'));assert.ok(script.includes('volcanicTiles(catalog.volcanoes)'));assert.ok(script.includes('FIRMS_CONCURRENCY'));assert.ok(script.includes('attempts=3'));assert.ok(script.includes('family:4'));assert.ok(script.includes('{forceIpv4:true}'));assert.ok(script.includes('minmagnitude=2.5'));assert.ok(!script.includes('for(const volcano of catalog.volcanoes){const detections=[]'));
assert.ok(script.includes('failed publication safety checks'),'only structurally unsafe partial data may stop the publication workflow');
for(const token of ['検出なし（安全判定ではありません）','複数衛星一致','最終成功観測','一部未取得','危険度ではなく、公的警戒色を変更しません'])assert.ok(ui.includes(token),token);
assert.ok(worker.includes('data/world/volcano-observations-v1.json'));
assert.ok(worker.includes('networkFirstLiveData'));
