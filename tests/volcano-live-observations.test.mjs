...
assert.ok(script.includes('/world/${FIRMS_DAY_RANGE}'));assert.ok(script.includes('attempts=3'));assert.ok(script.includes('minmagnitude=2.5'));assert.ok(!script.includes('for(const volcano of catalog.volcanoes){const detections=[]'));
