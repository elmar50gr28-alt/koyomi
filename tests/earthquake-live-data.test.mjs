...
assert.ok(worker.includes('live-earthquake-v15-immediate-native-render'),'service worker cache must be invalidated when immediate native rendering changes');
...
const app=await readFile(new URL('../app.html',import.meta.url),'utf8');assert.ok(app.includes("world-map-ui.js?v=earthquake-native-v15"),'app must bypass stale cached map modules');assert.ok(app.includes('world-map.css?v=earthquake-native-v15'),'app must bypass stale cached map styles');
