...
assert.ok(worker.includes('live-earthquake-v12-direct-trigger'),'service worker cache must be invalidated when the direct render trigger changes');
...
for(const token of ['world-live-earthquake-marker','world-live-earthquake-overlay','syncLiveEarthquakeDomMarkers','positionLiveEarthquakeMarkers','map.project','dataset.renderedMarkers','rendererVersion','v12-direct-trigger','setTimeout(()=>{liveMarkerSignature=','liveEarthquakeOverlay?.isConnected','地震の丸を描画できません','地球儀に${liveEarthquakeMarkers.length}件表示'])assert.ok(ui.includes(token),token);
...
const app=await readFile(new URL('../app.html',import.meta.url),'utf8');assert.ok(app.includes("world-map-ui.js?v=earthquake-overlay-v12"),'app must bypass stale cached map modules');assert.ok(app.includes('world-map.css?v=earthquake-overlay-v12'),'app must bypass stale cached map styles');
