...
assert.ok(worker.includes('live-earthquake-v13-native-positioning'),'service worker cache must be invalidated when native positioning changes');
...
for(const token of ['world-live-earthquake-marker','createLiveEarthquakeFallbackMarkers','verifyLiveEarthquakeRendering','queryRenderedFeatures','dataset.renderPath','new maplibreApi.Marker','setTimeout(()=>{liveMarkerSignature=','地震の丸を描画できません','標準マーカーで${liveEarthquakeMarkers.length}件表示'])assert.ok(ui.includes(token),token);
...
const app=await readFile(new URL('../app.html',import.meta.url),'utf8');assert.ok(app.includes("world-map-ui.js?v=earthquake-native-v13"),'app must bypass stale cached map modules');assert.ok(app.includes('world-map.css?v=earthquake-native-v13'),'app must bypass stale cached map styles');
