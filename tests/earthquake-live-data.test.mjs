...
import {EARTHQUAKE_DEPTH_LABEL_MIN_ZOOM,earthquakeDepthBand,earthquakeDepthBandLabel,earthquakeDepthIsDeep,earthquakeDepthLabelsVisible,earthquakeDepthMapLabel,earthquakeDepthPositionPercent} from '../src/world/earthquake-depth-presentation.js';
...
assert.equal(earthquakeDepthBand(0),'shallow');assert.equal(earthquakeDepthBand(69.9),'shallow');assert.equal(earthquakeDepthBand(70),'intermediate');assert.equal(earthquakeDepthBand(300),'deep');assert.equal(earthquakeDepthBand(500),'very-deep');assert.equal(earthquakeDepthBand(null),'unknown');
assert.equal(earthquakeDepthBandLabel(650),'非常に深い地震');assert.equal(earthquakeDepthMapLabel(620.4),'620 km');assert.equal(earthquakeDepthMapLabel(null),'深さ不明');assert.equal(earthquakeDepthPositionPercent(350),50);assert.equal(earthquakeDepthPositionPercent(900),100);assert.equal(earthquakeDepthIsDeep(300),true);assert.equal(earthquakeDepthIsDeep(299),false);
assert.equal(earthquakeDepthLabelsVisible(true,EARTHQUAKE_DEPTH_LABEL_MIN_ZOOM),true);assert.equal(earthquakeDepthLabelsVisible(true,EARTHQUAKE_DEPTH_LABEL_MIN_ZOOM-.01),false);assert.equal(earthquakeDepthLabelsVisible(false,9),false);
...
assert.ok(worker.includes('live-earthquake-v20-readable-depth'),'service worker cache must be invalidated for the readable depth presentation');
...
for(const token of ['earthquakeDepthToggle','earthquakeDeepFocusToggle','震源の深さ：OFF','深部を強調：OFF','showEarthquakeDepth','emphasizeDeepEarthquakes','selectedLiveEarthquakeId','syncEarthquakeDepthPresentation','earthquakeDepthCrossSectionHtml','earthquakeDepthPositionPercent','ON（拡大で表示）'])assert.ok(ui.includes(token),token);
...
const app=await readFile(new URL('../app.html',import.meta.url),'utf8');assert.ok(app.includes("world-map-ui.js?v=earthquake-native-v22-readable-attention-ui"),'app must bypass stale cached map modules');assert.ok(app.includes('world-map.css?v=earthquake-native-v21-readable-attention-ui'),'app must bypass stale cached map styles');
