...
assert.ok(worker.includes('live-earthquake-v9-cache-bypass'),'service worker cache must be invalidated when the map module URL changes');
...
const app=await readFile(new URL('../app.html',import.meta.url),'utf8');assert.ok(app.includes("world-map-ui.js?v=earthquake-dom-v9"),'app must bypass stale cached map modules');assert.ok(app.includes('world-map.css?v=earthquake-dom-v9'),'app must bypass stale cached map styles');
