...
assert.ok(worker.includes('live-earthquake-v14-all-native-markers'),'service worker cache must be invalidated when all-event native markers change');
...
for(const token of ['world-live-earthquake-marker','createLiveEarthquakeNativeMarkers','verifyLiveEarthquakeRendering','dataset.renderPath','all-native-markers','new maplibreApi.Marker','setTimeout(()=>{liveMarkerSignature=','地震の丸を描画できません','標準マーカーで全${liveEarthquakeMarkers.length}件表示'])assert.ok(ui.includes(token),token);
...
const app=await readFile(new URL('../app.html',import.meta.url),'utf8');assert.ok(app.includes("world-map-ui.js?v=earthquake-native-v14"),'app must bypass stale cached map modules');assert.ok(app.includes('world-map.css?v=earthquake-native-v14'),'app must bypass stale cached map styles');
