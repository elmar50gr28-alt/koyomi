...
assert.ok(worker.includes('live-earthquake-v16-theme-restore'),'service worker cache must be invalidated when theme restoration changes');
...
for(const token of ['world-live-earthquake-marker','createLiveEarthquakeNativeMarkers','syncLiveEarthquakeDomMarkers','dataset.renderPath','all-native-markers','new maplibreApi.Marker','setTimeout(()=>{liveMarkerSignature=','地震の丸を描画できません','標準マーカーで全${liveEarthquakeMarkers.length}件表示'])assert.ok(ui.includes(token),token);
assert.ok(!ui.includes('liveRenderTimer'),'native markers must not depend on a competing render timer');
for(const token of ["map.once('idle',restoreLiveMarkers)",'setTimeout(restoreLiveMarkers,500)',"theme==='earthquake'&&liveLayerOn"])assert.ok(ui.includes(token),token);
...
const app=await readFile(new URL('../app.html',import.meta.url),'utf8');assert.ok(app.includes("world-map-ui.js?v=earthquake-native-v16"),'app must bypass stale cached map modules');assert.ok(app.includes('world-map.css?v=earthquake-native-v16'),'app must bypass stale cached map styles');
